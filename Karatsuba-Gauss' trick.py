...
def recursive_multipication(x, y, n):
    ...
    if n == 1:
        ...
        two = recursive_multipication(b, d, min(len(str(b)), len(str(d))))
        three = recursive_multipication(a+b, c+d, min(len(str(a+b)), len(str(c+d))))      # gauss' trick
        if n%2==0:
            ans += (10**n)*one + 10**(n/2)*(three-one-two) + two
        else:
            ans += (10**(n-1))*one + 10**(n/2)*(three-one-two) + two
    return ans
